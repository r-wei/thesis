...
\frame{
	...
	%
	\only<1->
	{\begin{theorem}(Hochschild-Kostant-Rosenberg) Let $A$ be a regular, commutative algebra 
...
\frame{
	...
	\ar@/^2pc/[r]^{\textcolor{blue}{\Downarrow \phi_{n,1}}}_{f_{n,1}} 
	\ar@/_1pc/[r]^{\substack{\textcolor{blue}{\vdots}\\ f_{n,k_n}}}
	\ar@/_4pc/[rrrr]^{\substack{\mathlarger{\textcolor{blue}{\alpha}} \\\\}}_{ id_{A_n}}
	& A_0
	\ar@{.>}@/^4pc/[r] 
	\ar@{.>}@/^2pc/[r]_{\substack{\\ \\ \textcolor{blue}{\vdots}}} 
	\ar@{.>}@/_1pc/[r]
...
\frame{
	...
	\chi_\infty \to \mathcal{D}_\infty$}
	%
	\textbf{Solution:} Give these maps to 
	cogenerators to 
...
\frame{
	...
	%
	\only<5>
	{\bar{\tau}_{1!} \circ d (\phi \otimes \alpha)
	& = 
	d\circ \bar{\tau}_{1!}(\phi \otimes \alpha)\\}
	%
	\only<6-8>
...
\frame{
	...
	%
	\only<2>
	{\item They are homotopic via two different homotopies:
	$$\resizebox{1.1\vsize}{!}
	{\xymatrix{
	\underset{\substack{\\\\
