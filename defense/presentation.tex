...
\frame{
	...
	\begin{column}{0.3\textwidth} 
		${\scriptstyle \textcolor{blue}{\phi_{i,j}} \in C^\bullet(A_i, _{f_{i,j-1}}{A_{i+1}}_{f_{i,j}})}$ \\$\newline$
		\quad $d_B = \tilde{\delta} + d_\cup$
	\end{column}
	\end{columns}
	}
}
...
\frame{
	...
	$\alpha := a_0 \otimes \dots \otimes a_p$, $a_i \in A_0$
	\end{flushright}
...
\frame{
	...
	We have a given a \textcolor{blue}{$``$homotopically 
	...
	$``$A homotopically sheafy-cyclic 
	object in \textcolor{blue}{dg categories with a dg module}''
	%
	\only<2->
	{\begin{itemize}
	\only<2->
	{\item Rectify $\mathcal{F}$ to a dg functor 
	$U(\chi_\infty) \to \mathcal{D}_\infty$}
	  \only<2>
	  {$$Hom_{A_\infty-Cat}(\chi_\infty, \mathcal{D}_\infty) 
		= Hom_{DGCat}(U(\chi_\infty), \mathcal{D}_\infty)$$}
	%
	\only<3->	
	{\item Give a dg functor $Adj: \mathcal{D}_\infty \to \mathcal{D}_1$}
	\only<3>
	{\begin{align*}
	&\mathcal{D}_1^\bullet 
	  \big((B_1, C_1), (B_0, C_0) \big)
	  = \begin{Bmatrix}
	  F: B_1 \to B_0 \textrm{ dg functor}\\
	  F_!: F_\# C_1 \to C_0 \textrm{ linear map}\\
	  \textrm{``co-restriction of scalars''}
	  \end{Bmatrix}\\
	  &Hom_{B_1}(C_1, F^*C_0) = Hom_{B_0}(F_\#C_1, C_0)
	 \end{align*}}
	 %
	\only<4->
	{\item Give a dg functor $Cobar: 
	\mathcal{D}_1 \to \mathcal{E}$}
	\only<4>
	{\begin{align*}
	& Obj(\mathcal{E}) = 
	\{(\textrm{dg category, dg module})\}\\
	&\mathcal{E}^\bullet 
	  \big((A_1, M_1), (A_0, M_0) \big)
	  = \begin{Bmatrix}
	  F: A_1 \to A_0 \textrm{ dg functor}\\
	  F_!: M_1 \to F^*M_0 \textrm{ linear map}\\
	  \textrm{``restriction of scalars''}
	  \end{Bmatrix}
	 \end{align*}}
	\end{itemize}}
	%
	...
	{$$\newline \newline $$
	$$U(\chi_\infty)
	\xrightarrow{\textrm{rectified }\mathcal{F}}
	\mathcal{D}_\infty
	\xrightarrow{Adj}
	\mathcal{D}_1
	\xrightarrow{Cobar}
