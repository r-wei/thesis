...
\frame{
	...
	%
	\only<2->
	{A \underline{trace functor} on a category $\mathcal{C}$ in k-linear categories 
	gives a functor 
	$$\chi(\mathcal{C}) \to 
...
\frame{
	\textbf{Give an $A_\infty$-functor}
	\begin{align*}
	\chi_\infty(\mathcal{C}) 
	&\to 
	\mathcal{D}_\infty \\
...
\frame{
	...
	\textrm{Generating Morphisms:}\quad 
		...
		B(\mathcal{A}) \xrightarrow{\hat{\tau}_n}
		  B(\tau_n \mathcal{A})\\
		C(\mathcal{A}) 
		\xrightarrow{\textcolor{blue}{\tau_{n!} = 
		\delta_0^{*n-1}\tau_{1!}}} 
		  \hat{\tau}_n^* C(\tau_n\mathcal{A})
		\end{pmatrix}
	\end{align*}
...
\frame{
	...
	$\chi_\infty(\mathcal{C}) \to \mathcal{D}_\infty$, 
